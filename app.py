...
# 팀 라인업 편집 페이지
    ...
    fig.suptitle('팀 지표별 승수와의 연관관계')

    # OPS vs Wins
    axes[0, 0].scatter(team_stats_df['OPS'], results_df['wins'])
    axes[0, 0].set_xlabel('OPS')
    axes[0, 0].set_ylabel('Wins')
    axes[0, 0].set_title('OPS vs Wins')

    # SLG vs Wins
    axes[0, 1].scatter(team_stats_df['SLG'], results_df['wins'])
    axes[0, 1].set_xlabel('SLG')
    axes[0, 1].set_ylabel('Wins')
    axes[0, 1].set_title('SLG vs Wins')

    # OBP vs Wins
    axes[1, 0].scatter(team_stats_df['OBP'], results_df['wins'])
    axes[1, 0].set_xlabel('OBP')
    axes[1, 0].set_ylabel('Wins')
    axes[1, 0].set_title('OBP vs Wins')

    # ERA vs Wins
    axes[1, 1].scatter(team_stats_df['ERA'], results_df['wins'])
    axes[1, 1].set_xlabel('ERA')
    axes[1, 1].set_ylabel('Wins')
    axes[1, 1].set_title('ERA vs Wins')

    # WHIP vs Wins
    axes[2, 0].scatter(team_stats_df['WHIP'], results_df['wins'])
    axes[2, 0].set_xlabel('WHIP')
    axes[2, 0].set_ylabel('Wins')
    axes[2, 0].set_title('WHIP vs Wins')
